...
try:
                ...
                if int(month) == int(month_tar):
                    if int(day) == int(day_tar):
                        if int(start_hour) > 12:
                            ...
                            row_new = str(sast_start.date()) +' '+ str(sast_start.time()) + ' - ' + str(sast_end.date()) +' '+ str(sast_end.time()) + ' (SAST), ' + script_name + ', ' + block_id + ', ' + PI_name + ', ' + tar_name + ', ' + comment + '\n'
    ...
    text = "Today's spesific time targets list:\nSpecific time (start - end, SAST), Script name, Block ID, PI name, Target name, comment\n\n"
    for i in text_list:
        text += i
    ...
    root = tk.Tk()
    root.title("CheckDailySpecificTime.py")
    root.withdraw()

    messagebox_window = tk.Toplevel(root)
    messagebox_window.geometry("1200x600")

    messagebox_window.protocol("WM_DELETE_WINDOW", close_program)

    message_label = ttk.Label(messagebox_window, text=text, wraplength=1200-20)
